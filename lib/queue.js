...
Queue.prototype = {
  add: function(tree, _options) {
    ...
    this.promises[key].push(new RSVP.Promise(function(resolve) {
      setTimeout(function() {
        resolve();
      }, 10000);
    }));
  ...
  dependentPromisesFor: function(buildKey) {
    ...
    var depKeys = this.order.slice(0, keyIndex);
    var allPromises = this.promises;

    return depKeys.reduce(function(promises, key) {
      promises = promises.concat(allPromises[key]);
      return promises;
    }, []);
  }
};
